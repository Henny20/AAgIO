    ...
    public partial class MainWindow : Window
    {
        ...
        private void DoNTRIPSecondRoutine()
        {
            ...
            if (isNTRIP_Connecting)
            {
                if (ntripCounter > 29)
                {
                    var tmb = new TimedMessageBox(2000, "Connection Problem", "Not Connecting To Caster");
                    tmb.ShowDialog(this);
                    ReconnectRequest();
                }
            ...
            if (isNTRIP_RequiredOn || isRadio_RequiredOn)
            {
                ...
                //lblNTRIPBytes.Text = ((tripBytes >> 10)).ToString("###,###,### kb"); No Click
                lblNTRIPBytes.Content = ((tripBytes >> 10)).ToString("###,###,### kb"); 

                //Bypass if sleeping
                if (focusSkipCounter != 0)
                {
                    //update byte counter and up counter
                    if (ntripCounter > 59) btnStartStopNtrip.Content = (ntripCounter >> 6) + " Min";
                    else if (ntripCounter < 60 && ntripCounter > 22) btnStartStopNtrip.Content = ntripCounter + " Secs";
                    else btnStartStopNtrip.Content = "In " + (Math.Abs(ntripCounter - 22)) + " secs";
            ...
            {
                //pbarNtripMenu.Value = unchecked((byte)(tripBytes * 0.02));
                lblNTRIPBytes.Content = ((tripBytes >> 10)).ToString("###,###,### kb"); //No Click
        ...
        public void ConfigureNTRIP()
        {
            ...
            if (isNTRIP_RequiredOn || isRadio_RequiredOn || isSerialPass_RequiredOn)
            {
                btnStartStopNtrip.IsVisible = true;
                btnStartStopNtrip.IsVisible = true;
                lblWatch.IsVisible = true;
                lblNTRIPBytes.IsVisible = true;
                lblToGPS.IsVisible = true;
                lblMount.IsVisible = true;
                lblNTRIP_IP.IsVisible = true;
            }
            else
            {
                btnStartStopNtrip.IsVisible = false;
                btnStartStopNtrip.IsVisible = false;
                lblWatch.IsVisible = false;
                lblNTRIPBytes.IsVisible = false;
                lblToGPS.IsVisible = false;
                lblMount.IsVisible = false;
                lblNTRIP_IP.IsVisible = false;
            }

            btnStartStopNtrip.Content = "Off";
        }
        ...
        public void StartNTRIP()
        {
            if (isNTRIP_RequiredOn)
            {
                ...
                {
                    this.tmr = new DispatcherTimer();  //new System.Windows.Forms.Timer();
                    this.tmr.Interval = TimeSpan.FromMilliseconds(5000);
            ...
            else if (isRadio_RequiredOn)
            {
                if (!string.IsNullOrEmpty(Properties.Settings.Default.setPort_portNameRadio))
                {
                    ...
                    catch (Exception ex)
                    {
                        ...
                        var form = new TimedMessageBox(2000, "Error connecting to radio", $"{ex.Message}");
                ...
                if (!string.IsNullOrEmpty(Properties.Settings.Default.setPort_portNameRadio))
                {
                    ...
                    catch (Exception ex)
                    {
                       ...
                       // TimedMessageBox(2000, "Error connecting to Serial Pass", $"{ex.Message}");
                         var form = new TimedMessageBox(2000, "Error connecting to Serial Pass", $"{ex.Message}");
                         form.ShowDialog(this);
                    }
                }
            }
        ...
        private void IncrementNTRIPWatchDog()
        {
            ...
            if (sendGGAInterval > 0 && ntripCounter == 40) tmr.Interval = TimeSpan.FromMilliseconds(sendGGAInterval * 1000);
        }
        ...
        private void SendAuthorization()
        {
            ...
            try
            {
                if (!Properties.Settings.Default.setNTRIP_isTCP)
                {
                    ...
                    if (sendGGAInterval > 0) tmr.IsEnabled = true;
                }
                //say its connected
                isNTRIP_Connected = true;
                isNTRIP_Starting = false;
        ...
        private void ntripMeterTimer_Tick(object sender, EventArgs e)
        {
            ...
            if (rawTrip.Count == 0)
            {
                ntripMeterTimer.IsEnabled = false;
