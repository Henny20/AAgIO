...
namespace AAgIO.Views
{
    public partial class MainWindow : Window
    {
        ...
        public async void OpenIMUPort()
        {
            ...
            catch (Exception e)
            {
               ...
               // MessageBox.Show(e.Message + "\n\r" + "\n\r" + "Go to Settings -> COM Ports to Fix", "No Arduino Port Active");
                var msbox = MessageBoxManager.GetMessageBoxStandard(e.Message + "\n\r" + "\n\r" + "Go to Settings -> COM Ports to Fix", "No Arduino Port Active");
        ...
        public async void CloseIMUPort()
        {
            if (spIMU.IsOpen)
            {
                spIMU.DataReceived -= sp_DataReceivedIMU;
                ...
                catch (Exception e)
                {
                    ...
                    //MessageBox.Show(e.Message, "Connection already terminated??");
                     var msbox = MessageBoxManager.GetMessageBoxStandard(e.Message, "Connection already terminated??");
                 await msbox.ShowWindowDialogAsync(this);  
                }
        ...
        //open the Arduino serial port
        public async void OpenSteerModulePort()
        {
            if (!spSteerModule.IsOpen)
            {
        ...
        //close the machine port
        public async void CloseSteerModulePort()
        {
            if (spSteerModule.IsOpen)
            {
                spSteerModule.DataReceived -= sp_DataReceivedSteerModule;
        ...
        //open the Arduino serial port
        public async void OpenMachineModulePort()
        {
            if (!spMachineModule.IsOpen)
            {
        ...
        //close the machine port
        public async void CloseMachineModulePort()
        {
            if (spMachineModule.IsOpen)
            {
                spMachineModule.DataReceived -= sp_DataReceivedMachineModule;
                ...
                catch (Exception e)
                {
                    //WriteErrorLog("Closing Machine Serial Port" + e.ToString());
                   //MessageBox.Show(e.Message, "Connection already terminated??");
                     var msbox = MessageBoxManager.GetMessageBoxStandard(e.Message, "Connection already terminated??");      await msbox.ShowWindowDialogAsync(this); 
                }
        ...
        public async void CloseGPSPort()
        {
            //if (sp.IsOpen)
            {
                //spGPS.DataReceived -= sp_DataReceivedGPS;
        ...
        public async void CloseGPS2Port()
        {
            //if (sp.IsOpen)
            {
                spGPS2.DataReceived -= sp_DataReceivedGPS2;
        ...
        public async void CloseRtcmPort()
        {
            {
                try { spRtcm.Close(); }
                catch (Exception e)
